...
public interface IFreewarAPI {
	 ...
	 * {@link #login(String, String, EWorld)} was used it will stick to the browser
	 * set at method call. By that multiple instances using different browsers can
	 * be created.
	 * 
	...
	/**
	 * Hijacks the given session of a logged in account in <tt>Freewar</tt>. This
	 * allows a login without knowing the password which is especially useful for
	 * service applications. The resulting instance is the same as if regularly
	 * using {@link #login(String, String, EWorld)} provided the session is valid.
	 * If the session is not valid the method itself will not throw any exceptions
	 * as the session may become valid in the future by external force, however
	 * until then most further methods will likely throw exceptions. The API is
	 * capable of holding multiple {@link IFreewarInstance}s. If logging in
	 * different accounts of the same world, make sure to use different browsers.
	 * Else <tt>Freewar</tt> will automatically logout the first account. Set
	 ...
	 *            applications. It is maintained by <tt>Freewar</tt> and saved in a
	 *            cookie
	 * @param username
	 *            Username the session is valid for
	 * @param world
	 *            World the session is valid for
	 * @return Instance of the account after login or <tt>null</tt> if arguments are
	 *         invalid
	 */
	 ...
	 * multiple {@link IFreewarInstance}s. If logging in different accounts of the
	 * same world, make sure to use different browsers. Else <tt>Freewar</tt> will
	...
	/**
	 * Logs out from an account in <tt>Freewar</tt>. After logout the given instance
	 * is invalid and should not be used anymore.
	 * 
	 * @param instance
	 *            Instance to logout
	 * @param doQuitDriver
	 *            <tt>True</tt> if the browser driver of the given instance should
	 *            be quit at logout, <tt>false</tt> if not. If not quit the browser
	...
	/**
	 * Sets the capabilities to use for browsers. Using this one can set for example
	 * browser profiles to use, or the path to the binary of the browser.
	 * 
	 * @param capabilities
	...
	/**
	 * Shuts the API down, closing all remaining connections. This method does not
	 * necessarily logout from remaining {@link IFreewarInstance}s.
	 * 
	 * @param doQuitDriver
	 *            <tt>True</tt> if the browser drivers of instances should be quit
	 *            at shutdown, <tt>false</tt> if not. If not quit the browsers can
	 *            still be accessed by the user even after API shutdown.
	 */
